...
function screenHome() {
  const rows = statuses().map((s) => {
    ...
    const avg = rollingAverage(store.setResults(), s.paper);
    const avgTxt = avg == null ? 'no sets yet' : `avg ${avg.toFixed(1)}/10`;
...
function screenPaper(paper) {
  ...
  app.innerHTML = headerHTML + bannerHTML()
    ...
    + `<p class="progress">${sum.complete} of ${sum.total} topics complete${avg == null ? '' : ` · rolling average ${avg.toFixed(1)}/10`}</p>`
...
function screenTopic(topicId) {
  ...
  app.innerHTML = headerHTML + bannerHTML()
    ...
    + `<button class="btn ghost block" data-act="teach" data-topic="${esc(T.topicId)}">Teach me this</button>`
...
function screenDashboard() {
  ...
  const bars = ss.map((s) => {
    ...
    return `<div class="card flat"><div class="row"><span class="nm">${esc(PAPER_NAMES[s.paper])}</span>`
      + `<span class="progress">${complete}/${total} topics${avg == null ? '' : ` · ${avg.toFixed(1)}/10`}</span></div>`
  ...
  if (play.phase === 'ask') {
    ...
    if (!r.correct && r.diagnosis) {
      ...
      body += `<div class="diag"><div class="eyebrow">Likely cause</div><div class="cause">${esc(CAUSE_LABEL[d.cause] || d.cause)}</div>`
        ...
        + `<button class="btn ghost small" data-act="topic" data-id="${esc(topicIdForConcept(graph, inst.conceptIds[0]))}">Review the topic</button>`
        + '</div>';
    }
...
function finishSet() {
  ...
  app.innerHTML = headerHTML + bannerHTML()
    ...
    + `<p class="progress">Rolling average ${avg == null ? '—' : avg.toFixed(1)}/10 · this set was ${play.examShaped ? 'exam-shaped' : 'not fully exam-shaped (limited content so far)'}</p>`
    + (failed ? '<button class="btn ghost block" data-act="teach" data-topic="">Teach me what I missed</button>' : '')
...
// ---------- Teach Me This (client stub until the Vercel endpoint ships) ----------
function teachMe(topicId) {
  // The grounded Claude-API call via Vercel is the next increment; the button is placed now.
  toast('“Teach me this” is coming next — the grounded tutor is being wired up.');
  if (topicId) store.addTeachUse({ id: 'teach-' + Date.now(), topicId, at: Date.now(), sessionId: SESSION, stub: true });
}
...
document.addEventListener('click', (e) => {
  ...
  else if (act === 'topic') { location.hash = '#/topic/' + encodeURIComponent(t.getAttribute('data-id')); }
  else if (act === 'set') {
  ...
  else if (act === 'teach') { teachMe(t.getAttribute('data-topic')); }
