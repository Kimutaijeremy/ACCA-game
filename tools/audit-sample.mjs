...
w('');
w('> **State: Batch 0 repaired (2026-08-06).** All four required artifacts are now authored for these');
w('> three concepts — lesson, one-line nutshell, rung-floor practice set, and exactly one sealed item —');
w('> and `npm run items:check` reports 15/15 ACCEPTED. The sealed item is shown here for review but is');
w('> **never served in practice**. (FA-04 and FA-11 each had one surplus standard deferred under');
w('> breadth-before-depth; the deferred item lives in `src/content/items/deferred.js`.)');
w('');
w('---');
w('');
